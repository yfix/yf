...
// TODO: add support for sub-arrays and params
	...
	*/
	public function show($params = array()) {
		...
		if (strpos($main_style_css, PHP_EOL) === false && strlen($content_style_css) && preg_match('~^css/style.css?[0-9]{10}$~ims', $content_style_css)) {
			$this->add(WEB_PATH. tpl()->TPL_PATH. $main_style_css, 'url');
		} else {
			$this->add($main_style_css, 'inline');
		}
		// CSS from current module
		$module_css_path = $this->_find_module_css($_GET['object']);
		if ($module_css_path) {
			$this->add($module_css_path, 'file');
		}
		$out = array();
