...
class profy_main {
	...
	*/
	function init_conf_functions () {
		...
		$fwork_conf_path = dirname(dirname(__FILE__))."/share/functions/profy_conf.php";
	...
	*/
	function init_cache_functions () {
		...
		$fwork_cache_path	= dirname(dirname(__FILE__))."/share/functions/profy_cache.php";
...
###################
	...
	*/
	function fix_required_constants() {
		...
		$_GET["object"] = str_replace(array("profy_","-"), array("","_"), preg_replace("/[^a-z_\-0-9]*/", "", strtolower(trim($_GET["object"]))));
	...
	*/
	function _init_cur_user_info(&$OBJ) {
		...
		if (in_array(str_replace("profy_", "", get_class($OBJ)), (array)$this->_auto_info_skip_modules)) {
			return false;
		}
		if (MAIN_TYPE_ADMIN) {
