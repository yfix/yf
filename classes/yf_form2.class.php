...
// TODO: test how this will work with several forms
	...
	* Wrapper for template engine
	* Example template:
		{form_row("form_begin","form_action")}
		{form_row("text","login","Login")}
		{form_row("text","password","Password")}
		{form_row("text","first_name","First Name")}
		{form_row("text","last_name","Last Name")}
		{form_row("text","go_after_login","Url after login")}
		{form_row("box_with_link","group_box","Group","groups_link")}
		{form_row("active","active","Active")}
	{if("for_edit" eq 1)}
...
#		$errors = array();
	...
	function active($name, $desc = "", $more_param = "", $replace = array()) {
		if ($this->_chained_mode) {
			$replace = $this->_replace;
		}
		$_statuses = array(
	...
	function form_begin($name, $desc = "", $more_param = "", $replace = array()) {
		...
		$body = '<form method="post" action="'.$replace['form_action'].'" class="form-horizontal">';
		if ($this->_chained_mode) {
			$this->_body[] = $body;
			return $this;
		}
