...
class yf_utils
{
    ...
    public function error_back($where_go_back = 'javascript:history.back()', $what_to_say = 'error')
    {
    ...
    public function back($where_go_back = 'javascript:history.back()', $what_to_say = 'back')
    {
    ...
    public function process_url($url = '', $force_rewrite = false, $for_site_id = false)
    {
    ...
    public function _filter_text($body, $length = 0)
    {
    ...
    public function _show_avatar($user_id = 0, $user_name = '', $as_link = 0, $is_middle = 0, $only_img_src = 0, $force_link = '')
    {
        ...
        $avatar_path = _gen_dir_path($user_id, INCLUDE_PATH . SITE_AVATARS_DIR, 0, 0777) . (int) $user_id . ($is_middle ? '_m' : '') . '.jpg';
    ...
    public function _avatar_exists($user_id = 0, $is_middle = 0)
    {
    ...
    public function _get_age_from_birth($birth_date = '0000-00-00')
    {
        if (empty($birth_date) || $birth_date == '0000-00-00') {
            return false;
        }
        $tmp = explode('-', $birth_date);
        return (int) (date('Y') - $tmp[0] - (strtotime(date('Y') . '-' . $tmp[1] . '-' . $tmp[2]) > time() ? 1 : 0));
    }

    // Display user nick name (or name before all nicks will not be transfered)
    public function _display_name($user_info = [])
    {
    ...
    public function _format_date($input_date = '', $type = 'short')
    {
        ...
        $date_to_show = ! is_numeric($input_date) ? strtotime($input_date) : (int) $input_date;
    ...
    public function _day_suffix_eng($timestamp = 0)
    {
        if (empty($timestamp)) {
            $timestamp = time();
        }
    ...
    public function xsb_encode($string)
    {
        $temp = null;
    ...
    public function xsb_decode($string)
    {
    ...
    // Add login activity
    public function _add_login_activity()
    {
        if (MAIN_TYPE_ADMIN) {
            return false;
        }
    ...
    public function _prepare_phone($phone = '')
    {
     ...
     */
    public function smart_htmlspecialchars($html_text = '')
    {
        if ( ! strlen($html_text)) {
            return '';
        }
    ...
    public function array_replace_recursive($array_1, $array_2)
    {
    ...
    public function format_bbcode_text($body = '')
    {
        if (empty($body)) {
            return '';
        }
    ...
    public function printr($var, $do_not_echo = false)
    {
        ob_start();
        print_r($var);
    ...
    public function _debug_log($text, $log_level = false)
    {
    ...
    /**
     * Create path.
     * @param mixed $path_to_create
     * @param mixed $dir_mode
     * @param mixed $create_index_htmls
     * @param mixed $start_folder
     */
    public function _mkdir_m($path_to_create = '', $dir_mode = 0755, $create_index_htmls = 0, $start_folder = '')
    {
     ...
     * @param mixed $link
     */
    public function _mklink($target, $link)
    {
    ...
    public function _prepare_for_stpl_exec($source = '')
    {
        return preg_replace('/[^a-z0-9\-\_\s]/ims', '', $source);
    }

    // Display link to user's profile
    public function _profile_link($user_info = 0, $skip_get_array = [], $do_add_get = true)
    {
        if (IS_FRONT == 1) {
            return false;
        }
    ...
    public function _error_need_login($go_after_login = '')
    {
        // Hosting frontend
        if (IS_FRONT == 1) {
            return '';
    ...
    public function _output_cache_trigger($data = [])
    {
    ...
    public function _country_name($code = '')
    {
    ...
    public function _region_name($region_code = '', $country_code = '')
    {
    ...
    public function _email_link($user_id = 0, $skip_get_array = [], $do_add_get = true)
    {
    ...
    public function _prepare_members_link($url = '')
    {
        if (main()->USER_ID) {
            return $url;
        }
    ...
    public function _range($_start = 0, $_end = 10)
    {
        $data = [];
    ...
    public function _my_strip_tags($_text = '')
    {
    ...
    public function _rename($src_filename, $dest_filename)
    {
    ...
    public function _floatval($val = 0)
    {
        return (float) (str_replace(',', '.', $val));
    }

    // Useful explode with cleanup
    public function my_explode($string = '', $divider = "\n")
    {
        $result = explode("\n", trim($string));
    ...
    public function _exec_in_background($cmd)
    {
