...
#		'html5shiv'	=> '<!--[if lt IE 9]><script src="//cdnjs.cloudflare.com/ajax/libs/html5shiv/3.7/html5shiv.min.js" class="yf_core"></script><![endif]-->',
	...
	*/
	public function show($params = array()) {
		...
		if (strpos($main_script_js, PHP_EOL) === false && strlen($content_script_js) && preg_match('~^js/script.js?[0-9]{10}$~ims', $content_script_js)) {
			$this->add(WEB_PATH. tpl()->TPL_PATH. $main_script_js, 'url');
		} else {
			$this->add($main_script_js, 'inline');
		}
		// JS from current module
		$module_js_path = $this->_find_module_js($_GET['object']);
		if ($module_js_path) {
			$this->add($module_js_path, 'file');
		}
		$out = array();
