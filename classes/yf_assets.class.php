...
class yf_assets {
	...
	*/
	public function init_js($params = array()) {
		...
		if (strpos($main_script_js, "\n") === false && strlen($main_script_js) && preg_match('~^js/script.js\?[0-9]{10}$~ims', $main_script_js)) {
			$this->add_url(WEB_PATH. tpl()->TPL_PATH. $main_script_js, 'js');
		} else {
			$this->add_raw($main_script_js, 'js');
		}
	}
	...
	*/
	public function init_css($params = array()) {
		...
		if (strpos($main_style_css, "\n") === false && strlen($main_style_css) && preg_match('~^css/style.css\?[0-9]{10}$~ims', $main_style_css)) {
			$this->add_url(WEB_PATH. tpl()->TPL_PATH. $main_style_css, 'css');
		} else {
			$this->add_raw($main_style_css, 'css');
		}
	}
