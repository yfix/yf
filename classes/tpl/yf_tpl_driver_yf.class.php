...
#		}
	...
	*/
	function _process_js_css($string, $replace = array(), $name = '') {
		...
		$string = preg_replace_callback('/\{(?P<func>css|require_css|js|require_js|asset|jquery|angularjs|backbonejs|reactjs|emberjs|sass|less|coffee)\(\s*["\']{0,1}(?P<args>[^"\'\)\}]*?)["\']{0,1}\s*\)\}\s*(?P<content>.+?)\s*{\/(\1)\}/ims', function($m) use ($_this) {
			$func = $m['func'];
