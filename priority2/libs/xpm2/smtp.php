...
//					}else trigger_error('Invalid 2\'nd parameter type value, on class SMTP::addto()', 512);
					}else trigger_error('Invalid 2\'nd parameter type value, on class SMTP::addto()', E_USER_NOTICE);
//				}else trigger_error('Already exists, on class SMTP::addto()', 512);
				}else trigger_error('Already exists, on class SMTP::addto()', E_USER_NOTICE);
//			}else trigger_error('Invalid 1\'st parameter value, on class SMTP::addto()', 512);
			}else trigger_error('Invalid 1\'st parameter value, on class SMTP::addto()', E_USER_NOTICE);
//		}else trigger_error('Invalid 1\'st parameter type value, on class SMTP::addto()', 512);
		}else trigger_error('Invalid 1\'st parameter type value, on class SMTP::addto()', E_USER_NOTICE);
		return $ret;
...
//				}else trigger_error('Invalid 2\'nd parameter type value, on class SMTP::from()', 512);
				}else trigger_error('Invalid 2\'nd parameter type value, on class SMTP::from()', E_USER_NOTICE);
//			}else trigger_error('Invalid mail address format, on class SMTP::from()', 512);
			}else trigger_error('Invalid mail address format, on class SMTP::from()', E_USER_NOTICE);
//		}else trigger_error('Invalid first parameter type value, on class SMTP::from()', 512);
		}else trigger_error('Invalid first parameter type value, on class SMTP::from()', E_USER_NOTICE);
		return $ret;
					...
					if($rcvr = @fgets($sock, $this->max_sl)){
						if(substr($rcvr, 0, 3) == "250"){
							if(fputs($sock, 'QUIT'.$this->_crlf)){
								if(FUNC::is_connection($sock)){
// We do not want PHP error messages here
		...
		if(is_string($subject)){
			...
			if($subject != ""){
				...
				if(!($this->_toaddrs && count($this->_toaddrs) > 0)){
					$ver = false;
					trigger_error('You must set "To" e-mail address(es) using function "AddTo()", on class SMTP::send()', E_USER_NOTICE);
				}
				if(!($this->_atext || $this->_ahtml)){
					$ver = false;
