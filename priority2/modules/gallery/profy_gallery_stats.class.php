...
class profy_gallery_stats {
	...
	*/
	function _show_stats($MAIN_STPL = "", $ITEM_STPL = "", $NUM_ITEM = 0) {
		if(!empty($NUM_ITEM)){
			$this->GALLERY_OBJ->STATS_NUM_LATEST = $NUM_ITEM;
		}
// TODO: decide what to here if HIDE_TOTAL_ID enabled
		if (empty($MAIN_STPL)) {
			$MAIN_STPL = GALLERY_CLASS_NAME."/stats_main";
		}
		if (empty($GLOBALS['user_info']) && !empty($this->GALLERY_OBJ->_user_info)) {
			$GLOBALS['user_info'] = $this->GALLERY_OBJ->_user_info;
		}
		...
		$this->GALLERY_OBJ->_users_names = $users_names;
		// Prefetch folders infos
		$this->GALLERY_OBJ->_get_user_folders_for_ids($users_ids);
		// Prepare ids
		...
		// Prepare tags block
		$this->_tags = $this->GALLERY_OBJ->_show_tags($objects_ids, array("simple" => 1));

		// Process latest photos
		foreach ((array)$latest_photos_array as $A) {
			// Stop when number of users reached
			if (++$c1 > $this->GALLERY_OBJ->STATS_NUM_LATEST) {
				break;
			}
		...
		foreach ((array)$geo_latest_photos_array as $A) {
			// Stop when number of users reached
			if (++$c2 > $this->GALLERY_OBJ->STATS_NUM_LATEST) {
				break;
			}
		...
		$replace = array(
			...
			"show_own_gallery_link"	=> $this->GALLERY_OBJ->USER_ID ? "./?object=".GALLERY_CLASS_NAME."&action=show_gallery&id=".intval($this->GALLERY_OBJ->USER_ID)._add_get(array("page")) : "",
			"all_galleries_link"	=> "./?object=".GALLERY_CLASS_NAME."&action=show_all_galleries"._add_get(array("page")),
			"latest_photos"			=> $latest_photos,
			"num_latest"			=> intval($this->GALLERY_OBJ->STATS_NUM_LATEST),
			"latest_pages"			=> $latest_pages,
			"latest_total"			=> intval($latest_total),
			"geo_latest_photos"		=> $geo_latest_photos,
			"geo_num_latest"		=> intval($this->GALLERY_OBJ->STATS_NUM_LATEST),
			...
			"filter"				=> $this->GALLERY_OBJ->_show_filter(),
			"use_ajax"				=> 1,
...
// TODO: decide what to here if HIDE_TOTAL_ID enabled
		...
		if (!empty($_SESSION["gallery_filter"]["as_photos"])) {
			$this->GALLERY_OBJ->_SEARCH_AS_PHOTOS = 1;
		}
		// Special search type
		if ($this->GALLERY_OBJ->_SEARCH_AS_PHOTOS) {
...
//		$order_by_sql = " ORDER BY `p`.`priority` DESC, `num_photos` DESC ";
		...
		if (!empty($gallery_users_num_photos)) {
			$folders_by_users = $this->GALLERY_OBJ->_get_user_folders_for_ids(array_keys($gallery_users_num_photos));
		}
		// Prepare photos
		if (!empty($gallery_users_num_photos)) {
			$Q = db()->query(
		...
		foreach ((array)$gallery_users_num_photos as $user_id => $user_num_photos) {
			...
			foreach ((array)$photos_by_users[$user_id] as $_photo_id => $_photo_info) {
				$_fs_thumb_name = $this->GALLERY_OBJ->_photo_fs_path($_photo_info, $cur_photo_type);
				...
				$photos[$_photo_id] = array(
					...
					"img_src"	=> $this->GALLERY_OBJ->_photo_web_path($_photo_info, $cur_photo_type),
				);
			}
			// folders
			$folders = array();
...
// TODO: decide what to do here if HIDE_TOTAL_ID enabled
		...
		// Turn off count rewrite for "divide_pages"
//		$GLOBALS["PROJECT_CONF"]["divide_pages"]["SQL_COUNT_REWRITE"] = false;
		list($add_sql, $latest_pages, $latest_total) = common()->divide_pages(preg_replace("/ORDER BY .*?\$/ims", "ORDER BY NULL", $sql), $url, null, $this->GALLERY_OBJ->STATS_NUM_LATEST * 2);
		// Get top latest photos
		$Q = db()->query($sql. /*$order_by_sql.*/ $add_sql);
		while ($A = db()->fetch_assoc($Q)) {
			$latest_photos_array[$A["id"]] = $A;
		...
		if ($this->GALLERY_OBJ->ALLOW_TAGGING) {
			$_prefetched_tags = $this->GALLERY_OBJ->_get_tags(array_keys((array)$latest_photos_array));
		}
		// Process latest photos
		foreach ((array)$latest_photos_array as $A) {
	...
	*/
	function _show_latest_item ($photo_info = array(), $user_name = "", $ITEM_STPL = "") {
		...
		return $this->GALLERY_OBJ->_show_photo_item($photo_info, $params);
	}
	...
	*/
	function _show_latest_user_photos($user_info = array(), $only_featured = false, $latest_photos_array = array()) {
		...
		// Process tags
		$this->_tags = $this->GALLERY_OBJ->_show_tags(array_keys((array)$latest_photos_array));
		$user_name = _display_name($user_info);
		// Prefetch tags array
		if ($this->GALLERY_OBJ->ALLOW_TAGGING) {
			$_prefetched_tags = $this->GALLERY_OBJ->_get_tags(array_keys((array)$latest_photos_array));
	...
	*/
	function _show_latest($params = array()) {
// TODO: decide what to here if HIDE_TOTAL_ID enabled
		// Override page
		if (!empty($_GET["id"]) && empty($_GET["page"])) {
			$_GET["page"] = $_GET["id"];
