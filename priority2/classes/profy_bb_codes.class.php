...
class profy_bb_codes {
	...
	*/
	function _init () {
		...
		// BB codes for the "preg_replace" function
		$this->_preg_bb_codes = array(
			"/\[url=[\"']{0,1}([^\]]*)[\"']{0,1}\](.*?)\[\/url\]/ie"=> "'<a href=\"'.str_replace('&amp;','&','\$1').'\" target=\"blank\"".$nofollow.">\$2</a>'",
...
// TODO: add JS code here for email protection
		...
		if ($this->USE_CUSTOM_BB_CODES) {
			...
			foreach ((array)$custom_codes as $_custom_tag => $_info) {
				...
				$_regex = "/\[".$_custom_tag."[=]{0,1}([a-z0-9\_\-]+)".($_info["useoption"] ? "{0,1}" : "{0}")."\](.*?)\[\/".$_custom_tag."\]/ims";
	...
	*/
	function _process_text ($body = "", $no_smilies = false, $smilies_as_image = false) {
		...
		// Kill all tags (if exists ones)
		// !!! DO NOT USE _prepare_html here !!!
		$body = str_replace(array("<",">"), array("&lt;","&gt;"), $body);
		// Try to filter "bad words"
...
//					$body = preg_replace("/\[html\](.*?)\[\/html\]/imse",	"\$this->_geshi_highlight('\$1')",$body);
				...
				if (is_object($HIGHLIGHT_OBJ)) {
					$body = preg_replace("/\[sql\](.*?)\[\/sql\]/imse",		"\$HIGHLIGHT_OBJ->_regex_sql_tag(stripslashes('\$1'))",	$body);
					$body = preg_replace("/\[html\](.*?)\[\/html\]/imse",	"\$HIGHLIGHT_OBJ->_regex_html_tag(stripslashes('\$1'))",$body);
				}
			...
			if ($this->ENABLE_SMILIES && is_array($GLOBALS["_smiles_array"]) && empty($no_smilies)) {
				...
				if (!empty($GLOBALS["_SMILIES_CACHE"])) {
					...
					// Create smiles replace array (with caching)
...
// TODO: move HTML styles into template or separate option
		$output		= "";
		$c			= 0;
		$num_cols	= 0;
		// Process lines
