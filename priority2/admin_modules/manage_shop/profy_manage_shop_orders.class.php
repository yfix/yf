...
*/
class profy_manage_shop_orders {
	...
	*/
	function show_orders() {
		...
		$filter_sql = $this->PARENT_OBJ->USE_FILTER ? $this->_create_filter_sql() : "";
		...
		foreach ((array)$orders_info as $v){
			$items[] = array(
				...
				"sum"				=> $this->PARENT_OBJ->_format_price($v["total_sum"]),
				"user_link"		=> _profile_link($v["user_id"]),
				"user_name"	=> _display_name($user_infos[$v["user_id"]]),
				"status"			=> $v["status"],
				"delete_url"		=> "./?object=".$_GET["object"]."&action=delete_order&id=".$v["id"],
		...
		$replace = array(
			"items"		=> (array)$items,
			"pages"	=> $pages,
			"summ"		=> $this->PARENT_OBJ->_format_price($summ),
			"total"		=> intval($total),
			"filter"		=> $this->PARENT_OBJ->USE_FILTER ? $this->_show_filter() : "",
		);
		return tpl()->parse($_GET["object"]."/order_main", $replace);
	}
	...
	*/
	function view_order() {
		...
		if (!empty($products_ids)) {
			...
			$products_atts	= $this->PARENT_OBJ->_get_products_attributes($products_ids);
		}
		foreach ((array)$order_items as $_info) {
			...
			$products[$_info["product_id"]] = array(
				...
				"price"			=> $this->PARENT_OBJ->_format_price($_info["sum"]),
				"currency"	=> _prepare_html($this->PARENT_OBJ->CURRENCY),
		...
		$replace = my_array_merge($replace, array(
			...
			"total_sum"			=> $this->PARENT_OBJ->_format_price($order_info["total_sum"]),
			"user_link"			=> _profile_link($order_info["user_id"]),
			"user_name"		=> _display_name(user($order_info["user_id"])),
			"error_message"	=> _e(),
			"products"			=> (array)$products,
			"total_price"			=> $this->PARENT_OBJ->_format_price($total_price),
			"ship_type"			=> $this->PARENT_OBJ->_ship_types[$order_info["ship_type"]],
			"pay_type"			=> $this->PARENT_OBJ->_pay_types[$order_info["pay_type"]],
			"date"					=> _format_date($order_info["date"], "long"),
			"status_box"			=> $this->PARENT_OBJ->_box("status", $order_info["status"]),
			"back_url"				=> "./?object=".$_GET["object"]."&action=show_orders",
			"print_url"				=> "./?object=".$_GET["object"]."&action=show_print&id=".$order_info["id"],
	...
	*/
	function show_print() {
		...
		$_GET["id"] = intval($_GET["id"]);
		if ($_GET["id"]) {
			$order_info = db()->query_fetch("SELECT * FROM `".db('shop_orders')."` WHERE `id`=".intval($_GET["id"])." AND `user_id`=".intval($this->PARENT_OBJ->USER_ID));
		}
		if (empty($order_info)) {
			return _e("No such order");
		}
	$products_ids = array();
		...
		$total_price = $order_info["total_sum"];
		if($this->USER_ID) {
			$order_info = my_array_merge($this->PARENT_OBJ->_user_info, $order_info);
		}else {
			$order_info ["email"]= $order_info["email"];
			$order_info ["phone"]= $order_info["phone"];
		}
		$order_info = my_array_merge($this->PARENT_OBJ->COMPANY_INFO, $order_info);
		$replace2 = my_array_merge($order_info ,array(
			"id"		=> $_GET["id"],
			"products"		=> $products,
			"ship_cost"		=> $this->PARENT_OBJ->_format_price(0),
			"total_cost"		=> $this->PARENT_OBJ->_format_price($total_price),
			"password"		=> "", // Security!
		));
		...
		$replace = my_array_merge($replace2, array(
			"error_message"	=> _e(),
			"products"				=> $products,
			"ship_price"			=> $this->PARENT_OBJ->_format_price($this->PARENT_OBJ->_ship_types_names[$order_info["ship_type"]]),
			"total_price"			=> $this->PARENT_OBJ->_format_price($total_price),
			"order_no"				=> str_pad($order_info["id"], 8, "0", STR_PAD_LEFT),
			"hash"						=> _prepare_html($order_info["hash"]),
			"back_link"				=> "./?object=".$_GET["object"]."&action=show_orders",
			
		));
		return tpl()->parse($_GET["object"]."/order_print_invoice", $replace);
	}
	...
	*/
	...
	function _show_filter () {
		$replace = array(
			"save_action"	=> "./?object=".$_GET["object"]."&action=save_filter_order"._add_get(),
			"clear_url"		=> "./?object=".$_GET["object"]."&action=clear_filter_order"._add_get(),
		);
		...
		return tpl()->parse($_GET["object"]."/filter_order", $replace);
	}
	...
	function clear_filter ($silent = false) {
		...
		if (!$silent) {
			js_redirect("./?object=".$_GET["object"]."&action=show_orders"._add_get());
		}
	}
