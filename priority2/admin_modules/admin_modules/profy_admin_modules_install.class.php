...
* Installation code
*/
class profy_admin_modules_install {
	...
	function _import () {
		...
		if (isset($_POST["go"])) {
			...
			if (!common()->_error_exists()) {
				...
				} elseif ($xml_data["profy_install"]["ATTRIBUTES"]["type"] != "admin_module") {
...
//					"admin_files"		=> $admin_files,
				...
				$new_archive_name	= dirname($tmp_folder_path)."/profy_admin_module__".$_POST["module"].($this->PARENT_OBJ->USE_UNIQUE_TMP_DIR ? "_".substr(md5(microtime(true)), 0, 8) : "").".".($_POST["file_format"] == "zip" ? $_POST["file_format"] : "tar.".$_POST["file_format"]);
			...
			if (!common()->_error_exists()) {
				...
				$file_name = "profy_admin_module__".$_POST["module"].".".($_POST["file_format"] == "zip" ? $_POST["file_format"] : "tar.".$_POST["file_format"]);
				// Throw headers
