...
//	var $USE_FILTER		= false;
	...
	*/
	function _init () {
		...
		$this->_cats_for_select	= $this->CATS_OBJ->_prepare_for_box(SHOP_CLASS_NAME."_cats", 0);
	...
	*/
	function show () {
		...
		$replace = array(
			"items"					=> $items,
			"products_url"		=> "./?object=".$_GET['object']."&action=manage_products",
	...
	function manage_products () {
		...
		foreach ((array)$products_info as $v){
			$replace2 = array(
				...
				"quantity"		=> $v["quantity"],
				"active"			=> $v["active"],
				"edit_url"			=> "./?object=".$_GET["object"]."&action=edit&id=".$v["id"],
				"delete_url"	=> "./?object=".$_GET["object"]."&action=delete&id=".$v["id"],
				"view_url"		=> "./?object=".$_GET["object"]."&action=view&id=".$v["id"],
				"activate_url"	=> "./?object=".$_GET["object"]."&action=activate&id=".$v["id"],
			);
		...
		return tpl()->parse($_GET["object"]."/main_main", $replace);
	}
	...
	*/
	function add () {
		if (!empty($_POST)) {
			...
			if (!common()->_error_exists()) {
				...
				if (!empty($_FILES)) {
					...
					$rez_upload = $this->_upload_image ($product_id, $product_name);
					$sql_array = array(
							"image"	=> 1,
					);
				} 
		...
		$replace = array(
			...
			"back_url"					=> "./?object=".$_GET["object"]."&action=manage_products",
	...
	*/
	function edit () {
		...
		if (isset($_POST["product_related"])) {
			...
			return js_redirect("./?object=".$_GET['object']."&action=manage_products");
		}
		if ($product_info["image"] == 0) {
			...
			foreach((array)$image_files as $filepath) {
				...
				$i =  $rezult["content"];
				$delete_image_url ="./?object=".$_GET['object']."&action=delete_image&id=".$product_info["id"]."&name=".$product_info["url"]."&key=".$i;
	...
	*/
	function statistic () {
		...
		$sql = "SELECT SUM(`total_sum`) FROM `".db('shop_orders')."`";
		$total_sum = db()->query_fetch($sql);
		$sql = "SELECT COUNT(*) FROM `".db('shop_orders')."`";
		$total_order = db()->query_fetch($sql);
		$sql = "SELECT COUNT(*) FROM `shop_shop_products`";
		$total_prod = db()->query_fetch($sql);
		$sql = "SELECT COUNT(*) FROM `".db('shop_orders')."` WHERE `status` = 'pending'";
		$total_order_pending = db()->query_fetch($sql);
		$sql = "SELECT SUM(`total_sum`) FROM `".db('shop_orders')."` WHERE `status` = 'shipped'";
		$total_sum_shipped = db()->query_fetch($sql);
		$replace = array(
	...
	* Upload image
	*/
	function _upload_image ($product_id, $product_name) {
		$i = 1;
...
;		$i = $this->_chek_filed ($img_path, $product_id, $product_name, $i);
			$img_path = $this->products_img_dir . $mpath.$product_name."_".$product_id."_".$i.$this->FULL_IMG_SUFFIX.".jpg";
	...
	function _chek_filed ($path, $product_id, $product_name, $i) {
		 if (file_exists($path)) {
			$i = $i +1;
			$img_path = $this->products_img_dir.$product_name."_".$product_id."_".$i.$this->FULL_IMG_SUFFIX.".jpg";
			$this->_chek_filed ($img_path, $product_id, $product_name, $i);
		} 
		return $i;
	}
	...
	* Delete image
	*/
	function _delete_image ($id, $name, $k) {
		$dirs = sprintf("%06s",$id);
		$dir2 = substr($dirs,-3,3);
		$dir1 = substr($dirs,-6,3);
		$mpath = $dir1."/".$dir2."/";
		$image_files = $this->DIR_OBJ->scan_dir($this->products_img_dir.$mpath, true, "/".$name."_".$id."_".$k.".+?jpg"."/");
		foreach((array)$image_files as $filepath) {
			unlink($filepath);
		}
		$image_files = $this->DIR_OBJ->scan_dir($this->products_img_dir.$mpath, true, "/".$name."_".$id.".+?.jpg"."/");
		if (!$image_files ){
			$sql_array = array(
					"image"	=> 0,
				);
	...
	*/
	function delete_image () {
		...
		$this->_delete_image($_GET["id"], $_GET["name"], $_GET["key"]);
		return js_redirect($_SERVER["HTTP_REFERER"]);
	}
	...
	*/
	function activate () {
		if ($_GET["id"]){
			...
			db()->UPDATE(db('shop_products'), array(
				"active"		=> $active,
			),
			"`id`='".intval($_GET["id"])."'" 
			);
	...
	*/
	...
	function _prepare_filter_data () {
		...
		$this->_filter_name	= "manage_products_filter";
	...
	function clear_filter ($silent = false) {
		...
		if (!$silent) {
			js_redirect("./?object=".$_GET["object"]."&action=manage_products"._add_get());
		}
	}
