	...
	function _import () {
		...
		if (isset($_POST["go"])) {
			...
			if (!common()->_error_exists()) {
				...
				} elseif ($xml_data["yf_install"]["ATTRIBUTES"]["type"] != "user_module") {
	...
	function _export () {
		...
		if (isset($_POST["go"])) {
			...
			if (!common()->_error_exists()) {
				...
				$new_archive_name	= dirname($tmp_folder_path)."/yf_user_module__".$_POST["module"].($this->PARENT_OBJ->USE_UNIQUE_TMP_DIR ? "_".substr(md5(microtime(true)), 0, 8) : "").".".($_POST["file_format"] == "zip" ? $_POST["file_format"] : "tar.".$_POST["file_format"]);
			...
			if (!common()->_error_exists()) {
				...
				$file_name = "yf_user_module__".$_POST["module"].".".($_POST["file_format"] == "zip" ? $_POST["file_format"] : "tar.".$_POST["file_format"]);
				// Throw headers
