...
* Installation code
*/
class profy_user_modules_install {
	...
	*/
	function _init () {
		...
		$this->PARENT_OBJ	= module("user_modules");
	}
	...
	*/
	function _uninstall () {
		...
		$module_class_path = INCLUDE_PATH.USER_MODULES_DIR.$_GET["id"].CLASS_EXT;
		if (file_exists($module_class_path)) {
			$files[$module_class_path] = $module_class_path;
		}
		// Sub modules
		$sub_modules_dir = INCLUDE_PATH.USER_MODULES_DIR.$_GET["id"]."/";
		...
		// Get first site theme
		foreach ((array)$this->PARENT_OBJ->_sites_info->info as $site_id => $SITE_INFO) {
			$site_theme_name = $this->PARENT_OBJ->_sites_info->_get_site_current_theme($SITE_INFO);
			break;
		}
		// Try to find templates
		if (!empty($site_theme_name)) {
			$module_templates_dir = INCLUDE_PATH.tpl()->_THEMES_PATH.$site_theme_name."/".$_GET["id"]."/";
	...
	function _import () {
		...
		if (isset($_POST["go"])) {
			...
			if (!common()->_error_exists()) {
				...
				if (!isset($xml_data["profy_install"]) || !isset($xml_data["profy_install"]["ATTRIBUTES"]["type"])) {
					common()->_raise_error(t("Cant find package info in XML data"));
				} elseif ($xml_data["profy_install"]["ATTRIBUTES"]["type"] != "user_module") {
					common()->_raise_error(t("Wrong package type"));
				} elseif (empty($xml_data["profy_install"]["name"]["VALUE"]) || $PACKAGE_NAME != $xml_data["profy_install"]["name"]["VALUE"]) {
					common()->_raise_error(t("Wrong package name"));
				} elseif (empty($xml_data["profy_install"]["files"]["VALUE"])) {
			...
			if (!common()->_error_exists()) {
				if (isset($this->PARENT_OBJ->_modules[$PACKAGE_NAME])) {
					common()->_raise_error(t("User module \"@name\" already exists. If you want to install current package - you need to uninstall old one first.", array("@name" => $PACKAGE_NAME)));
				}
			}
			// Process package contents
			if (!common()->_error_exists()) {
				// Verify package files
				foreach ((array)$xml_data["profy_install"]["files"]["file_name"] as $file_info) {
			...
			if (!common()->_error_exists()) {
				...
				// Raise error only if we have templates in the package
				if (!empty($templates_list) && empty($site_theme_name)) {
					common()->_raise_error(t("Cannot get theme name from the current site"));
				}
			}
			...
			if (!common()->_error_exists()) {
				foreach ((array)$xml_data["profy_install"]["install_queries"]["query"] as $query_info) {
					db()->query($query_info["VALUE"]);
				}
			}
			// Run custom installer file
			if (!common()->_error_exists()) {
				$custom_install_file = $xml_data["profy_install"]["install_file"]["VALUE"];
			...
			if (!common()->_error_exists()) {
				db()->INSERT("user_modules", array(
					...
					"description"	=> _es($xml_data["profy_install"]["description"]["VALUE"]),
					"version"		=> _es($xml_data["profy_install"]["version"]["VALUE"]),
					"author"		=> _es($xml_data["profy_install"]["author"]["VALUE"]),
					"active"		=> 0,
				));
			}
	...
	function _export () {
		...
		if (isset($_POST["go"])) {
			...
			if (!common()->_error_exists()) {
				...
				// Get first found site info
				foreach ((array)$this->PARENT_OBJ->_sites_info->info as $site_id => $SITE_INFO) {
					$site_theme_name = $this->PARENT_OBJ->_sites_info->_get_site_current_theme($SITE_INFO);
					$site_real_path	= $SITE_INFO["REAL_PATH"];
					$site_web_path	= $SITE_INFO["WEB_PATH"];
					// Stop here
					break;
				}
				// Create export dir for user templates
				$tmp_templates_path = $tmp_folder_path."templates/user/";
				_mkdir_m($tmp_templates_path);
				// Try to find templates for the specified module
				$templates_path = INCLUDE_PATH.tpl()->_THEMES_PATH.$site_theme_name."/".$_POST["module"]."/";
				...
				$replace2 = array(
					"install_type"		=> _prepare_html("user_module"),
				...
				$new_archive_name	= dirname($tmp_folder_path)."/profy_user_module__".$_POST["module"].($this->PARENT_OBJ->USE_UNIQUE_TMP_DIR ? "_".substr(md5(microtime(true)), 0, 8) : "").".".($_POST["file_format"] == "zip" ? $_POST["file_format"] : "tar.".$_POST["file_format"]);
			...
			if (!common()->_error_exists()) {
				...
				$file_name = "profy_user_module__".$_POST["module"].".".($_POST["file_format"] == "zip" ? $_POST["file_format"] : "tar.".$_POST["file_format"]);
				// Throw headers
