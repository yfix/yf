...
class yf_related_content
{
    ...
    public function _process($params = [])
    {
        ...
        $PAST_ONLY = $params['past_only'] ? (int) ((bool) $params['past_only']) : $this->DEF_PARAMS['PAST_ONLY'];
        if ( ! $FIELD_DATE || $FIELD_DATE == -1) {
            $PAST_ONLY = false;
        }
        $RECORDS_LIMIT = $params['limit'] ? (int) ($params['limit']) : $this->DEF_PARAMS['RECORDS_LIMIT'];
        ...
        $THRESHOLD = $params['thold'] ? (int) ($params['thold']) : $this->DEF_PARAMS['THRESHOLD'];
        $WEIGHT_TEXT = $params['weight_body'] ? (int) ($params['weight_body']) : $this->DEF_PARAMS['WEIGHT_TEXT'];
        $WEIGHT_TITLE = $params['weight_title'] ? (int) ($params['weight_title']) : $this->DEF_PARAMS['WEIGHT_TITLE'];
        // Additional fields
        $WEIGHT_ADD_1 = $params['weight_add_1'] ? (int) ($params['weight_add_1']) : $this->DEF_PARAMS['WEIGHT_ADD_1'];
        $WEIGHT_ADD_2 = $params['weight_add_2'] ? (int) ($params['weight_add_2']) : $this->DEF_PARAMS['WEIGHT_ADD_2'];
        // TODO: complete these
        $WEIGHT_TAG = $params['weight_tag'] ? (int) ($params['weight_tag']) : $this->DEF_PARAMS['WEIGHT_TAG'];
        $WEIGHT_CAT = $params['weight_cat'] ? (int) ($params['weight_cat']) : $this->DEF_PARAMS['WEIGHT_CAT'];
        ...
        $sql =
			...
			FROM ( 
				...
				WHERE ' . ($WHERE_COND ? $WHERE_COND : '1') . ' 
					AND ' . $FIELD_ID . ' != ' . (int) ($SOURCE_ARRAY[$FIELD_ID])
        ...
        if ($WHAT_TO_RETURN == 'stpl') {
            ...
            foreach ((array) $data as $k => $v) {
                $data2[$k] = [
                    ...
                    'href' => process_url('./?object=' . $_GET['object'] . '&action=' . $_GET['action'] . '&id=' . (int) ($v[$FIELD_ID])),
                ];
            }
            $replace = [
                'data' => $data2,
     ...
     * @param mixed $text
     */
    public function _get_keywords_from_text($text = '')
    {
     ...
     * @param mixed $fields
     */
    public function _prepare_fields_param($fields = '')
    {
        if (is_array($fields)) {
