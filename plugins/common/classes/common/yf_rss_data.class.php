...
class yf_rss_data
{
    ...
    public function show_rss_page($data = [], $params = [])
    {
        ...
        $rss->descriptionTruncSize = ! empty($params['feed_trunc_size']) ? (int) ($params['item_trunc_size']) : 500;
        ...
        foreach ((array) $data as $A) {
            ...
            // optional params
            $item->descriptionTruncSize = ! empty($params['item_trunc_size']) ? (int) ($params['item_trunc_size']) : $this->DESC_TRUNC_SIZE;
            ...
            if ( ! empty($A['enclosure']) && is_array($A['enclosure'])) {
                ...
                $item->enclosure->length = (int) ($E['length']);
    ...
    public function fetch_data($params = [])
    {
        ...
        $Q = db()->query('SELECT * FROM ' . db('rss_items') . ' ORDER BY feed_id,add_date DESC LIMIT ' . (int) (count((array) $this->_rss_feeds) * $NUM_ITEMS));
        while ($A = db()->fetch_assoc($Q)) {
            $this->_latest_cached_items[$A['feed_id']][$A['cache_md5']] = $A;
        }
        // Process feeds
    ...
    public function _get_rss_feed_array($rss_url = '', $feed_ttl = 3600, $num_items = 15)
    {
        ...
        if (is_object($cur_ch_image)) {
            $_ch_image = [
                ...
                'width' => (int) ($cur_ch_image->getWidth()),
            ];
        }
        $output['channel'] = [
     ...
     * @param mixed $rssDoc
     */
    public function _feed_encoding($rssDoc)
    {
     ...
     * @param mixed $text
     */
    public function _decode_text($text = '')
    {
