...
class yf_forum_compact_view {
	...
	*/
	function _topic_repliers() {
		...
		// Do get repliers ids
		$Q = db()->query(
			'SELECT COUNT(id) AS num_posts,user_id,user_name FROM '.db('forum_posts').' 
			WHERE topic='.intval($topic_id).' AND id != '.intval($this->_topic_info['first_post_id']).' 
				AND user_id != 0 '.(!FORUM_IS_ADMIN ? ' AND status="a" ' : '').' 
			GROUP BY user_id 
		...
		// Throw output
		echo t('Topic posts by users').':<br />'.PHP_EOL;
		echo $body;
	}
