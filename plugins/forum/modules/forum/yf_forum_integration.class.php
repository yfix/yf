...
class yf_forum_integration {
	...
	*/
	function _site_map_items ($OBJ = false) {
		...
		$Q = db()->query($sql);
		while ($A = db()->fetch_assoc($Q)) {
			$sql = 'SELECT COUNT(id) AS num FROM '.db('forum_topics').' WHERE forum='.intval($A['id']).' AND approved=1 AND pinned=0';
		...
		$Q = db()->query($sql);
		while ($A = db()->fetch_assoc($Q)) {
			$sql = 'SELECT COUNT(id) AS num FROM '.db('forum_posts').' WHERE topic='.intval($A['id']).' AND status="a"';
			$B = db()->query_fetch($sql);
