	...
	*/
	function _show_post_form ($post_info = array(), $topic_info = array(), $new_topic = false, $edit_post = false) {
		...
		if (!FORUM_USER_ID && !module('forum')->SETTINGS['ALLOW_GUESTS_POSTS']) {
			_re('Guests are not allowed to make posts!');
	...
	function _save_post () {
		...
		if (module('forum')->SETTINGS['USE_BAN_IP_FILTER']) {
			if (db()->query_num_rows('SELECT ip FROM '.db('bannedip').' WHERE ip="'._es(common()->get_ip()).'"')) {
				return module('forum')->_show_error('Your IP address was banned!. For more details <a href=\'./?object=faq&action=view&id=16\'>click here</a>');
			}
		}
		...
		if (!common()->_error_exists()) {
			...
			if ($ACT != 'new_topic') {
				...
				if (empty($topic_info['id'])) _re('No such topic!');
			}
		}
		// Check if forum or topic closed
		if (!FORUM_IS_ADMIN) {
		...
		// Anti-flood filter
		if (!common()->_error_exists()) {
			$POSSIBLE_FLOOD	= db()->query_num_rows(
				'SELECT id FROM '.db('forum_posts').' WHERE created > '.(time() - module('forum')->SETTINGS['ANTISPAM_TIME']).' AND poster_ip="'.common()->get_ip().'" LIMIT 1'
			);
			if ($POSSIBLE_FLOOD) {
				_re('Possible flood detected! Try again later!');
			}
		}
		// Check required fields
		if (!common()->_error_exists()) {
			if ($ACT == 'new_topic' && empty($_POST['title'])) {
				_re('Topic title required!');
			}
			// Prepare text
...
//			$_POST['text'] = wordwrap($_POST['text'], 75, ''.PHP_EOL, 1);
		...
		if (!common()->_error_exists()) {
			...
			if ($QUOTE_SPAM) {
				_re('Possible quote spam! Please enter your text not just quote!');
			}
		}
		...
		if (!common()->_error_exists()) {
			...
			if (!empty($new_topic_id)) {
				...
				// Verify new topic
				if (empty($topic_info)) {
					_re('Error while creating new topic! Please contact site admin!');
		...
		if (!common()->_error_exists()) {
			...
			if (!empty($new_post_id)) {
				...
				// Verify new post
				if (empty($post_info)) {
					_re('Error while creating new post! Please contact site admin!');
