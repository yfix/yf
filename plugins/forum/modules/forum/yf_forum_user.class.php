...
//			return module('forum')->_show_error('Disabled by the site admin!');
		...
		if (!empty($user_info['id']) && !module('forum')->SETTINGS['HIDE_USERS_INFO']) {
			...
			if (!empty($user_info['user_posts'])) {
				...
				list($most_forum_id, $most_forum_posts) = db()->query_fetch(
					"SELECT forum AS `0`, COUNT(id) AS 1 FROM ".db('forum_posts')." WHERE user_id=".intval($user_info["id"])." AND status='a' GROUP BY forum ORDER BY 1 DESC LIMIT 1"
				);
