...
class yf_comments_manage
{
    ...
    public function _add($params = [])
    {
        ...
        if (count((array) $_POST) > 0 && ! isset($_POST['_not_for_comments'])) {
            ...
            if ( ! common()->_error_exists() && module('comments')->ANTI_FLOOD_TIME && MAIN_TYPE_USER) {
                ...
                if ( ! empty($FLOOD_DETECTED)) {
                    _re('Please wait %num seconds before post comment.', ['%num' => (int) (module('comments')->ANTI_FLOOD_TIME - (time() - $FLOOD_DETECTED['add_date']))]);
                }
            }
        ...
        if (empty($_POST['go']) || ! empty($error_message)) {
            ...
            $replace = [
                ...
                'object_id' => (int) $OBJECT_ID,
                'use_captcha' => (int) ((bool) module($_GET['object'])->USE_CAPTCHA),
                ...
                'js_check' => (int) ((bool) module('comments')->JS_TEXT_CHECKING),
    ...
    public function _edit($params = [])
    {
        ...
        $_GET['id'] = (int) ($_GET['id']);
        $comment_info = db()->query_fetch('SELECT * FROM ' . db('comments') . ' WHERE id=' . (int) ($_GET['id']));
