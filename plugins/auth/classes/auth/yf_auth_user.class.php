...
class yf_auth_user
{
    ...
    public function init()
    {
        ...
        if ($this->STORE_ONLINE_USERS && $this->ONLINE_AUTO_CLEAN && rand(1, 10) == 1) {
            db()->_add_shutdown_query('DELETE FROM ' . db('online') . ' WHERE (' . time() . '-time) > ' . (int) ($this->ONLINE_AUTO_CLEAN));
        }
        ...
        if ($this->STORE_ONLINE_USERS) {
            ...
            if ( ! isset($online_users)) {
                $online_users = [];
                $sql = 'SELECT id, user_id FROM ' . db('online') . (MAIN_TYPE_USER ? ' WHERE type != "admin"' : '') . ($this->ONLINE_MAX_IDS ? ' LIMIT ' . (int) ($this->ONLINE_MAX_IDS) : '');
    ...
    public function _do_login_with_encrypted()
    {
        ...
        $user_info = db()->get('SELECT *, MD5(`password`) AS md5_pswd FROM ' . db('user') . ' WHERE id=' . (int) $user_id);
     ...
     */
    public function _is_user_inactive($user_info = [])
    {
        if (empty($user_info)) {
            return true;
        }
     ...
     * @param mixed $user_info
     */
    public function _is_user_hidden($user_info = [])
    {
        if (empty($user_info)) {
            return true;
        }
     ...
     * Alias.
     * @param mixed $user_info
     * @param mixed $no_redirect_on_success
     */
    public function auto_login($user_info = [], $no_redirect_on_success = false)
    {
     ...
     * @param mixed $user_info
     */
    public function _update_user_info_on_login($user_info)
    {
    }
     ...
     */
    public function _process_cookie()
    {
     ...
     * Unset user cookie.
     */
    public function _cleanup_cookie()
    {
    ...
    public function _check_multi_accounts()
    {
        ...
        $data = db()->query_fetch('SELECT * FROM ' . db('check_multi_accounts') . ' WHERE user_id=' . (int) ($_SESSION[$this->VAR_USER_ID]));
        ...
        if (empty($data)) {
            db()->insert_safe('check_multi_accounts', [
                'user_id' => (int) ($_SESSION[$this->VAR_USER_ID]),
     ...
     * @param mixed $value
     */
    public function _set_special_cookie($value = '')
    {
     ...
     * Update online info.
     */
    public function _update_online_info()
    {
        if ( ! $this->STORE_ONLINE_USERS) {
            return false;
        }
        ...
        $data = [
            'id' => _es(session_id()),
            'user_id' => (int) ($_SESSION[$this->VAR_USER_ID]),
            'user_group' => (int) ($_SESSION[$this->VAR_USER_GROUP_ID]),
            'time' => time(),
            'type' => _es(MAIN_TYPE),
     ...
     * Ref codes.
     */
    public function _catch_ref_codes()
    {
        if ( ! $this->CATCH_REF_CODES) {
            return false;
        }
    ...
    {
        $path = $path === null ? $this->COOKIE_PATH : $path;
        $domain = $domain === null ? $this->COOKIE_DOMAIN : $domain;
        $secure = $secure === null ? $this->COOKIE_SECURE : $secure;
        $httponly = $httponly === null ? $this->COOKIE_HTTPONLY : $httponly;
