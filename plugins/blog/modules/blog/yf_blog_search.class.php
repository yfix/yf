...
// TODO
		...
		$filter_sql = module('blog')->USE_FILTER ? module('blog')->_create_filter_sql() : "";
		if (empty($filter_sql)) {
			$filter_sql = " ORDER BY num_posts DESC ";
		}
		// Give handling to the specific method if needed
		if (module('blog')->_SEARCH_AS_POSTS) {
		...
		$per_page = !empty($params["per_page"]) ? $params["per_page"] : module('blog')->VIEW_ALL_ON_PAGE;
		...
		foreach ((array)$blogs_infos as $user_id => $cur_blog_info) {
			...
			$view_allowed = module('blog')->_privacy_check($cur_blog_info["privacy"], 0, $user_id);
			// Force avatar link
			...
			$replace2 = array(
				...
				"rss_posts_button"	=> module('blog')->_show_rss_link("./?object=".'blog'."&action=rss_for_single_blog&id=".$user_info["id"], "RSS feed for blog: ".(!empty($cur_blog_info["blog_title"]) ? $cur_blog_info["blog_title"] : $user_name."'s blog")),
			);
		...
		$replace = array(
			...
			"filter"	=> module('blog')->_show_filter(),
...
// TODO
		...
		list($add_sql, $pages, $total) = common()->divide_pages(str_replace("SELECT *", "SELECT id", $sql), $path, null, module('blog')->VIEW_ALL_ON_PAGE);
		// Get contents from db
		$Q = db()->query($sql. $add_sql);
		...
		foreach ((array)$posts_infos as $post_id => $cur_post_info) {
			...
			// Check privacy permissions
			$view_allowed = module('blog')->_privacy_check($cur_post_info["privacy"], 0, $user_id);
			// Prepare found text
			$found_text = $cur_post_info["text"];
			if (strlen($found_text) > module('blog')->POST_TEXT_PREVIEW_LENGTH) {
				$found_text = _truncate($found_text, module('blog')->_SEARCH_AS_POSTS);
			}
			$found_text = _prepare_html($found_text);
			$found_text = nl2br(highlight($found_text, module('blog')->_SEARCH_AS_POSTS));
			// Force avatar link
			...
			$replace2 = array(
				...
				"rss_posts_button"	=> module('blog')->_show_rss_link("./?object=".'blog'."&action=rss_for_single_blog&id=".$user_info["id"], "RSS feed for blog: ".(!empty($cur_post_info["blog_title"]) ? $cur_post_info["blog_title"] : $user_name."'s blog")),
			);
...
*/
		...
		if (!empty($friends_ids)) {
			...
			list($add_sql, $pages, $total, $counter) = common()->divide_pages($sql, null, null, module('blog')->FRIENDS_POSTS_PER_PAGE);
			$Q = db()->query($sql. $add_sql);
			while ($A = db()->fetch_assoc($Q)) {
		...
		foreach ((array)$posts_array as $post_info) {
			...
			$replace2 = array(
				...
				"rss_posts_button"	=> module('blog')->_show_rss_link("./?object=".'blog'."&action=rss_for_single_blog&id=".$post_info["user_id"], "RSS feed for blog: ".(!empty($this->_users_blog_settings[$post_info["user_id"]]["blog_title"]) ? $this->_users_blog_settings[$post_info["user_id"]]["blog_title"] : _display_name($this->_users_infos[$post_info["user_id"]])."'s blog")),
			);
		...
		$replace = array(
			...
			"rss_friends_button"=> module('blog')->_show_rss_link("./?object=".'blog'."&action=rss_for_friends_posts&id=".$_GET["id"], "RSS feed for friends posts for blog: ".(!empty($this->_users_blog_settings[$_GET["id"]]["blog_title"]) ? $this->_users_blog_settings[$_GET["id"]]["blog_title"] : _display_name($this->_users_infos[$_GET["id"]])."'s blog")),
		);
