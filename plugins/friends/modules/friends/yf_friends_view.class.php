...
class yf_friends_view {
	...
	function view_all_friends () {
		...
		$PER_PAGE = module('friends')->ALL_FRIENDS_PER_PAGE;
		// Get user friends ids array
		$friends_ids_1 = module('friends')->_get_user_friends_ids($user_info["id"]);
		...
		foreach ((array)$users_array as $cur_user_info) {
			...
			$replace2 = array(
				...
				"delete_link"		=> $IS_OWN_PAGE ? "./?object=".'friends'."&action=delete&id=".$cur_user_info["id"] : "",
				"handshake_link"	=> "./?object=".'friends'."&action=request_handshake_form&id=".$cur_user_info["id"],
				"is_mutual_friend"	=> isset($friend_of_ids[$cur_user_info["id"]]) ? 1 : 0,
				"need_div"			=> !(++$c2 % module('friends')->VIEW_ALL_PER_LINE) ? 1 : 0,
			);
			$items .= tpl()->parse('friends'."/view_all_friends_item", $replace2);
		}
		// Process template
		$replace = array(
			"items"					=> $items,
			...
			"all_handshake_request"	=> "./?object=".'friends'."&action=all_handshake_request",
			"friends_posts"			=> "./?object=".'friends'."&action=friends_posts",
			"friends_groups"		=> "./?object=".'friends'."&action=friends_groups",
		);
	...
	function view_all_friend_of () {
		...
		// Get number of records per page
		$PER_PAGE = module('friends')->ALL_FRIEND_OF_PER_PAGE;
		// Get users friend of ids
		$friend_of_ids = module('friends')->_get_users_where_friend_of($user_info["id"]);
		// Process them
		...
		foreach ((array)$users_array as $cur_user_info) {
			...
			$replace2 = array(
				...
				"user_avatar"			=> _show_avatar($cur_user_info["id"], $cur_user_info, 1),
				"user_details_link"		=> _profile_link($cur_user_info["id"]),
				"add_to_friends_link"	=> $IS_OWN_PAGE && !isset($my_friends_ids[$cur_user_info["id"]]) ? "./?object=".'friends'."&action=add&id=".$cur_user_info["id"] : "",
				"is_mutual_friend"		=> isset($my_friends_ids[$cur_user_info["id"]]) ? 1 : 0,
				"need_div"				=> !(++$c2 % module('friends')->VIEW_ALL_PER_LINE) ? 1 : 0,
			);
			$items .= tpl()->parse('friends'."/view_all_friend_of_item", $replace2);
		}
		$replace = array(
			"items"							=> $items,
			"pages"							=> trim($pages),
			...
			"user_avatar"					=> _show_avatar($user_info["id"], $user_info, 1),
			"user_details_link"				=> _profile_link($user_info["id"]),
			"all_handshake_request_to_you"	=> "./?object=".'friends'."&action=all_handshake_request_to_you",
		);
	...
	function _show_friends_for_profile ($user_info = array(), $MAX_SHOW_ITEMS = 0) {
		...
		$NUM_ITEMS = !empty($MAX_SHOW_ITEMS) ? $MAX_SHOW_ITEMS : module('friends')->FOR_PROFILE_NUM_FRIEND_OF;
		// Get user friends ids array
		$friends_ids_1 = module('friends')->_get_user_friends_ids ($user_info["id"]);
		$GLOBALS['user_friends_ids'] = $friends_ids_1;
		...
		foreach ((array)$users_array as $cur_user_info) {
			...
			$replace2 = array(
				...
				"need_div"			=> !(++$c2 % module('friends')->FOR_PROFILE_PER_LINE) ? 1 : 0,
			);
			$items .= tpl()->parse('friends'."/for_profile_friends_item", $replace2);
		}
		...
		$replace = array(
			"items"		=> $items,
			"total"		=> count($friends_ids_1),
			"more_link"	=> "./?object=".'friends'."&action=view_all_friends&id=".intval($user_info["id"]),
		);
	...
	function _show_friend_of_for_profile ($user_info = array(), $MAX_SHOW_ITEMS = 0) {
		...
		// Get number of records to display
		$NUM_ITEMS = !empty($MAX_SHOW_ITEMS) ? $MAX_SHOW_ITEMS : module('friends')->FOR_PROFILE_NUM_FRIENDS;
		// Get users friend of ids
		$friend_of_ids = module('friends')->_get_users_where_friend_of($user_info["id"]);
		...
		foreach ((array)$users_array as $cur_user_info) {
			...
			$items .= tpl()->parse('friends'."/for_profile_friend_of_item", $replace2);
		}
		// Stop here if no items is found
		$items = trim($items);
		if (empty($items)) {
		...
		$replace = array(
			"items"		=> $items,
			"total"		=> count($friend_of_ids),
			"more_link"	=> "./?object=".'friends'."&action=view_all_friend_of&id=".intval($user_info["id"]),
		);
		return tpl()->parse('friends'."/for_profile_friend_of_main", $replace);
	}
	...
	*/
	function friends_posts(){
		...
		module('friends')->sql_array = array(
		...
		foreach ((array)module('friends')->sql_array as $key => $value){
			$post_type_select[$key] = $key;
		}
		
		if(!empty($_SESSION["post_type_select_box"])){
			$sql = str_replace("SELECT 'name'", "SELECT '".$_SESSION["post_type_select_box"]."' ", module('friends')->sql_array[$_SESSION["post_type_select_box"]]);
		}else{
			foreach ((array)module('friends')->sql_array as $_k => $_v) {
		...
		foreach ((array)$friends_posts as $post){
			...
			if ($post["name"] == "comment") {
				...
				$post_link = "./?object=".$comment_info["object_name"]."&action=".module('friends')->_comments_actions[$comment_info["object_name"]]."&id=".$comment_info["object_id"]."#cid_".$comment_info["id"];
			} elseif (!empty($post["name"])) {
				$post_link = module('friends')->_map_post_urls[$post["name"]].$post["id"];
			}
			$replace2 = array(
				"bg_class"	=> !(++$i % 2) ? "bg1" : "bg2",
				"id"		=> intval($post["id"]),
			...
			$items.= tpl()->parse('friends'."/friends_posts_item", $replace2);
		}
		
		$replace = array(
			"form_action"			=> "./?object=".'friends'."&action=".$_GET["action"],
			"items"					=> $items,
			"pages"					=> $pages,
		...
		return tpl()->parse('friends'."/friends_posts_main", $replace);
	}
}
