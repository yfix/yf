...
class yf_manage_shop_revisions
{
     ...
     * @param mixed $db_table
     */
    public function check_revision($function, $id, $db_table)
    {
        if (empty($function) || empty($id) || empty($db_table)) {
            return false;
        }
        if ( ! is_array($id) && (int) $id) {
            $ids = [(int) $id];
        }
    ...
    {
        $rev = db()->get('SELECT * FROM ' . db('shop_revisions') . ' WHERE id=' . (int) ($_GET['id']));
        if ( ! $rev) {
            return false;
        }
        $sql = 'SELECT * FROM ' . db('shop_revisions') . ' WHERE item_id=' . (int) ($rev['item_id']) . ' AND action !=\'\' ORDER BY id DESC';
        return table($sql, [
