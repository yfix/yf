...
//                $this->SAVE_PATH = PROJECT_PATH."uploads/tmp/";
        ...
        $this->SUPPLIER_ID = (int) db()->get_one('SELECT supplier_id FROM ' . db('shop_admin_to_supplier') . ' WHERE admin_id=' . (int) (main()->ADMIN_ID));
        set_time_limit(0);
    }
...
//                $rar = 'unrar e '.$full_archive_name.' '.$EXTRACT_PATH;
        ...
        if ($ext == 'rar') {
            common()->rar_extract($archive_name, $EXTRACT_PATH);
        }
        if ($ext == 'zip') {
            common()->zip_extract($archive_name, $EXTRACT_PATH);
        }
        if ($ext == 'tar' || $ext == 'gz') {
            exec($$ext);
        }
