...
class yf_manage_shop_product_images
{
    ...
    public function _product_image_upload($product_id)
    {
        ...
        foreach ((array) $_FILES['image']['tmp_name'] as $v) {
            ...
            $A = db()->query_fetch('SELECT COUNT(*) AS `cnt` FROM ' . db('shop_product_images') . ' WHERE product_id=' . (int) $product_id . ' AND is_default=1 AND active=1');
            if ($A['cnt'] == 0) {
                $A = db()->query_fetch('SELECT `id` FROM ' . db('shop_product_images') . ' WHERE `product_id`=' . (int) $product_id . ' ORDER BY `id` DESC');
