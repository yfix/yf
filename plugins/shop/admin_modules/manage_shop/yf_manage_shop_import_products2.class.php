...
class yf_manage_shop_import_products2
{
    ...
    protected function _fputcsv($file, array $fields, $delimiter = ',', $enclosure = '"', $mysql_null = false)
    {
        ...
        $result = fwrite($file, implode($delimiter, $output) . "\n");
        return  $result;
    }
    ...
    {
        if (empty($options) || empty($key) || empty($cols)) {
            return  null;
        }
    ...
    protected function _upload_item__import_test($options)
    {
        ...
        if ($action == 'update') {
            ...
            if ( ! empty($rules)) {
                foreach ($rules['include'] as $key) {
                    if (isset($fields_keys[$key])) {
                        $keys[$key] = $fields_keys[$key];
                    }
                }
            }
            $fields_keys = $keys;
        ...
        foreach ($items as $index => $item) {
            ...
            list($exists, $many, $found) = $this->_db_exists(
                'shop_products',
                ['where' => $where]
            );
            if ($action == 'update') {
