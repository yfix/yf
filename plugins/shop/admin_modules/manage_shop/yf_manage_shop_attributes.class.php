...
class yf_manage_shop_attributes
{
    public function attributes()
    {
        ...
        return table($items, [
            ...
            ->text('title')
            ->text('options')
            ->btn_edit('', './?object=' . main()->_get('object') . '&action=attribute_edit&id=%d', ['no_ajax' => 1])
    ...
    public function attribute_add()
    {
        if (main()->is_post()) {
            ...
            if ( ! common()->_error_exists()) {
                ...
                db()->INSERT('shop_productparams', db()->es([
                    'title' => $_POST['title'],
                ]));
                ...
                foreach ($value_list as $val) {
                    // insert new
                    db()->INSERT('shop_productparams_options', [
                        'productparams_id' => $insert_id,
                        'title' => $val,
                    ]);
                }

                common()->admin_wall_add(['shop product attribute added: ' . $_POST['title'], $insert_id]);
                return js_redirect('./?object=' . main()->_get('object') . '&action=attributes');
            }
        }
        $form_fields = ['name', 'type', 'value_list', 'default_value', 'order', 'comment'];
    ...
    public function attribute_edit()
    {
        ...
        if (main()->is_post()) {
            ...
            if ( ! common()->_error_exists()) {
                ...
                db()->UPDATE('shop_productparams', db()->es([
                    'title' => $_POST['title'],
                ]), 'id=' . $_GET['id']);
    ...
    {
        $_GET['id'] = (int) ($_GET['id']);
        $field_info = db()->query_fetch('SELECT * FROM ' . db('shop_productparams') . ' WHERE id = ' . (int) ($_GET['id']));
        if (empty($field_info)) {
            return _e('no field');
        }
        if ($_GET['id']) {
            ...
            common()->admin_wall_add(['shop product attribute deleted: ' . $_GET['id'], $_GET['id']]);
        }
        if ($_POST['ajax_mode']) {
    ...
    public function _attributes_html($object_id = 0, $only_selected = false)
    {
        ...
        $category_id = (int) (module('manage_shop')->ATTRIBUTES_CAT_ID);
        ...
        foreach ((array) $attributes as $_attr_id => $_info) {
            ...
            foreach ((array) $_info['value_list'] as $_val_id => $_value) {
                ...
                $data[$_item_id] = [
                    ...
                    'id' => $_item_id,
                    'attr_checked' => (int) ((bool) $selected_info['is_selected'][$_val_id]),
    ...
    public function _attributes_save($object_id = 0)
    {
        ...
        foreach ((array) $attributes as $_attr_id => $_info) {
            ...
            if ( ! isset($fields_values[$_attr_id])) {
                db()->INSERT('shop_product_attributes_values', [
                    'category_id' => $category_id,
                    'object_id' => $object_id,
                    'field_id' => (int) $_attr_id,
                    'value' => _es($option_values),
                    'add_value' => _es($value_prices),
                ]);
            } else {
                db()->UPDATE('shop_product_attributes_values', [
                    'value' => _es($option_values),
                    'add_value' => _es($value_prices),
                ], 'category_id = ' . $category_id . ' AND object_id = ' . $object_id . ' AND field_id = ' . (int) $_attr_id);
            }
        }
        return true;
    }
