...
// TODO: convert cp ...* to the "find . | xargs cp ..." to avoid errors like "argument list too long"
    ...
    public function file_exists($server_info = [], $path = '')
    {
        ...
        if (strlen($path)) {
            ...
            $result = (bool) (int) (_class('ssh')->exec($server_info, $command));
        ...
        foreach ((array) $m[0] as $id => $_matched_all) {
            ...
            if ($_type == 'd') {
                if ($level === null || $level > 0) {
                    // Merge result
                    foreach ((array) _class('ssh')->scan_dir($server_info, $item_name, $pattern_include, $pattern_exclude, $level === null ? $level : $level - 1) as $_path => $_info) {
                        $files[$_path] = $_info;
                    }
                }
            }
    ...
    public function chmod($server_info = [], $path = '', $new_mode = null, $recursively = false)
    {
        ...
        $path = _class('ssh')->_prepare_path($path);
        if ( ! strlen($path) || $new_mode === null || ! is_numeric($new_mode)) {
            return false;
        }
        // Do not allow to change folders with less than 1 level deep from "/",
        // for example: deny for "/var",
