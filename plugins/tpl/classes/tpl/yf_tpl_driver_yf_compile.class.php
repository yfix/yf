...
class yf_tpl_driver_yf_compile
{
     ...
     */
    public function _compile_prepare_ifs(array $m)
    {
     ...
     * @param mixed $cond
     */
    public function _cond_sub_array($cond)
    {
        $pos = strpos($cond, '.');
        if ($pos === false) {
    ...
    public function _compile_foreach($m)
    {
        ...
        if ( ! $foreach_data_tag) {
            $foreach_data_tag = 'is_array(' . $foreach_arr_tag . ') ? ' . $foreach_arr_tag . ' : $this->_range_foreach(' . (is_numeric($foreach_arr_name) ? (int) $foreach_arr_name : $foreach_arr_tag) . ')';
        }
