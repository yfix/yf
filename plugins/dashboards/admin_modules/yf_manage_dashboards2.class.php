...
class yf_manage_dashboards2
{
    ...
    public function show()
    {
        return table('SELECT * FROM ' . db('dashboards2'))
            ->text('name')
            ->text('type')
            ->func('lock', function ($value, $extra, $row_info) {
                return $value == 1 ? '<span class="label label-success">YES<span>' : '<span class="label label-danger">NO<span>';
            })
            ->btn_view()
            ->btn_edit()
            ->btn_clone()
    ...
    public function parse_structure()
    {
        main()->NO_GRAPHICS = true;
    ...
    {
        $id = $_GET['id'];
        $ds_info = db()->get('SELECT * FROM ' . db('dashboards2') . ' WHERE name="' . db()->es($id) . '" OR id=' . (int) $id);
        if ( ! $ds_info['id']) {
    ...
    public function active()
    {
        ...
        if ( ! empty($ds_info['id'])) {
            db()->update('dashboards2', ['active' => (int) ! $ds_info['active']], 'id=' . (int) ($_GET['id']));
    ...
    public function lock()
    {
        ...
        if ( ! empty($ds_info['id'])) {
            db()->update('dashboards2', ['lock' => (int) ! $ds_info['lock']], 'id=' . (int) ($_GET['id']));
    ...
    public function edit()
    {
        ...
        $a = db()->query_fetch('SELECT * FROM ' . db('dashboards2') . ' WHERE id=' . (int) ($_GET['id']));
