...
class yf_db_ddl_parser_mysql
{
     ...
     * @param mixed $params
     */
    public function create_fk_line($name = '', $v = [], $params = [])
    {
        if (is_array($name)) {
            $v = $name;
            $name = $v['name'];
                        ...
                        foreach ((array) $v2['sub_tree'] as $k3 => $v3) {
                            ...
                            if ($v3['expr_type'] == 'data-type') {
                                ...
                                if ($next1['expr_type'] == 'reserved') {
                                    $default = (string) ($next1['base_expr']);
     ...
     * @param mixed $type
     */
    public function _get_int_def_length($type)
    {
        $a = [
            'tinyint' => 3,
            'smallint' => 5,
     ...
     * @param mixed $name
     */
    public function escape_table_name($name = '')
    {
        $name = trim($name);
        if ( ! strlen($name)) {
            return false;
