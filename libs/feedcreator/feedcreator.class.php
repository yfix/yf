...
FeedCreator class v1.7.2-of (modified by of)
...
define("FEEDCREATOR_VERSION", "FeedCreator 1.7.2-of");
...
//			$content = htmlspecialchars($this->rawFieldContent);
// Added by of
			$content = htmlentities($this->rawFieldContent, ENT_QUOTES, "UTF-8");
			if ($this->truncSize and is_int($this->truncSize)) {
				$result = FeedCreator::iTrunc($content,$this->truncSize);
			} else {
				$result = $content;
			}
		}
...
class UniversalFeedCreator extends FeedCreator {
	 ...
	 *			"mbox", "RSS0.91", "RSS1.0", "RSS2.0", "OPML", "ATOM0.3", "HTML", "JS"
	 ...
	 * 
	 * @param	string	format	format the feed should comply to. Valid values are:
	 *			"mbox", "RSS0.91", "RSS1.0", "RSS2.0", "OPML", "ATOM", "ATOM0.3", "HTML", "JS"
	...
	*
	* @param   string   format   format the feed should comply to. Valid values are:
	*	   "mbox", "RSS0.91", "RSS1.0", "RSS2.0", "OPML", "ATOM0.3".
...
// Added by of
		if ($this->self_link) {
			$feed.= "		<atom:link href=\"".$this->self_link."\" rel=\"self\" type=\"application/rss+xml\" />\n";
		}
		$now = new FeedDate();
...
// valid format strings are: RSS0.91, RSS1.0, RSS2.0, MBOX, OPML, ATOM1.0, HTML, JS
