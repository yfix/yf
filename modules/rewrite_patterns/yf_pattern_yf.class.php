...
class yf_pattern_yf {
	function _get($A) {
		if ($A['task'] == 'login' || $A['task'] == 'logout') {
			...
			if (!empty($A['object'])) {
				$u[] = "{$A['object']}";
				if (empty($A['action'])) $A['action'] = 'show';
				$u[] = "{$A['action']}";
				if (!empty($A['id'])) $u[]= "{$A['id']}";
			}
			$u = implode("/",$u);
		}
		...
		$arr = $A;
		$arr_out = array();
		unset($arr['object']);unset($arr['action']);unset($arr['host']);unset($arr['id']);
	...
	function _parse ($host,$url,$query) {
		...
		if (!empty($url[2])) $s.="&id={$url[2]}";

		if ($s != '') {
			parse_str($s,$arr);
			foreach ((array)$query as $k=>$v) $arr[$k] = $v;
		}
		return $arr;
	}
