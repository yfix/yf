...
function install_show_html($page = 'form', $vars = array(), $errors = array()) {
	ob_start();
?>
<!DOCTYPE html>
<html>
...
<?php
	foreach ((array)install_get_form_keys() as $name => $desc) {
...
function install_get_default_web_path() {
...
function install_get_form_keys() {
	return array(
...
function install_get_form_defaults() {
	return array(
		...
		'install_web_path'					=> install_get_default_web_path(),
...
function install_prepare_vars() {
	$vars = array(
		'FORM_ACTION'	=> $_SERVER['PHP_SELF'],
	);
	$defaults = install_get_form_detaults();
	foreach ((array)install_get_form_keys() as $k => $desc) {
...
function install_write_db_setup() {
...
function install_write_user_index_php() {
...
function install_write_admin_index_php() {
...
function install_import_base_db_structure() {
	$import_tables = array(
		"activity_types",
		"countries",
		"forum_groups",
...
function install_import_demo_data() {
...
function install_write_htaccess($rewrite_enabled = true) {
	if ($rewrite_enabled) {
...
function install_set_admin_login_pswd() {
...
function install_copy_project_skeleton() {
	_class("dir")->copy_dir(INSTALLER_PATH.'data/', PROJECT_PATH, '', '/\.(svn|git)/');
}
/////////////////////////////
if (empty($_POST)) {
	install_show_html('form', install_prepare_vars());
	exit();
}
...
*/
if ($errors) {
	install_show_html('form', install_prepare_vars(), $errors);
	exit();
}
install_set_php_conf();
install_init_yf_core();
install_write_db_setup();
install_write_user_index_php();
install_write_admin_index_php();
#install_import_base_db_structure();
if ($_POST['install_checkbox_demo_data']) {
	install_import_demo_data();
}
install_write_htaccess($_POST['install_checkbox_rw_enabled']);
install_set_admin_login_pswd();
install_copy_project_skeleton();
#if ($_POST['install_checkbox_debug_info']) {
#	$body .= common()->show_debug_info();
#}
$vars["install_log"] = $body;
install_show_html('results', $vars);

echo common()->show_debug_info();
echo ++$i.") mem:".memory_get_usage().",mpeak:".memory_get_peak_usage()."<br>\n";
