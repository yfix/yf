...
<?php if ($replace['link'] != '') {
    ?>
			<a href="<?php echo $replace['link']; ?>"><?php if ($replace['icon_class'] != '') {
        ?><i class="icon icon-<?php echo $replace['icon_class']; ?> fa fa-<?php echo $replace['icon_class']; ?>"></i> <?php
    } ?><?php echo $replace['name']; ?></a>
<?php
} else {
        ?>
			<?php if ($replace['icon_class'] != '') {
            ?><i class="icon icon-<?php echo $replace['icon_class']; ?> fa fa-<?php echo $replace['icon_class']; ?>"></i> <?php
        } ?><?php echo $replace['name']; ?>
<?php
    } ?>
...
<?php
	...
	<?php if ($replace['next_level_diff'] != '0') {
            ?>
		<?php $__f_total = count((array) $replace['next_level_diff']);
            foreach (is_array($replace['next_level_diff']) ? $replace['next_level_diff'] : range(1, (int) $replace['next_level_diff']) as $_k => $_v) {
                $__f_counter++; ?></ul><?php
            } ?>
	<?php
        } ?>
