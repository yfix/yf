...
class class_core_events_test extends yf\tests\wrapper
{
    ...
    {
        $out = null;
        $this->assertFalse(events()->has_listeners('testme'));
        events()->listen('testme', function ($in) use (&$out) {
            $out = $in;
        });
        $this->assertTrue(events()->has_listeners('testme'));
        $this->assertNull($out);
    ...
    public function test_nested()
    {
        $out = null;
        $this->assertFalse(events()->has_listeners('testme'));
        $this->assertFalse(events()->has_listeners('testme_nested1'));
    ...
    public function test_nested3()
    {
        ...
        $this->assertFalse(events()->has_listeners('testme'));
        $this->assertFalse(events()->has_listeners('testme_nested1'));
        $this->assertFalse(events()->has_listeners('testme_nested2'));
        events()->listen('testme', function ($in) use (&$out) {
            events()->listen('testme_nested1', function ($in2) use (&$out) {
    ...
    {
        events()->listen('testme.1', function () {
            return '111';
        });
        events()->listen('testme.*', function () {
            return '222';
        });
        events()->listen('testme.3', function () {
            return '333';
        });
