...
LEFT JOIN `sexy_user` AS `u` 
	ON `ads`.`user_id` = `u`.`id` 
LEFT JOIN `sexy_category` AS `c` 
	ON `ads`.`cat_id` = `c`.`id` 
LEFT JOIN `sexy_activity_total` AS `act` 
	ON `ads`.`user_id` = `act`.`user_id` 
LEFT JOIN `sexy_reput_total` AS `reput` 
...
FROM `sexy_user` AS `u` 
LEFT JOIN `sexy_activity_total` AS `act` ON `u`.`id` = `act`.`user_id` 
LEFT JOIN `sexy_reput_total` AS `reput` ON `u`.`id` = `reput`.`user_id`
LEFT JOIN `sexy_ads` AS `ads` ON `u`.`id` = `ads`.`user_id`
WHERE 
	`u`.`sex` = 'female'
	AND `ads`.`cat_id` = 98
ORDER BY 
