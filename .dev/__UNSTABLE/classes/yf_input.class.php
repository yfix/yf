 ...
 * (Originally inspired from CodeIgniter 2.1)
 */
class yf_input {
...
#		log_message('debug', 'Input Class Initialized');
		...
		$this->_allow_get_array	= (conf('allow_get_array') === TRUE);
		$this->_enable_xss	= (conf('global_xss_filtering') === TRUE);
		$this->_enable_csrf	= (conf('csrf_protection') === TRUE);

		$this->security = _class('security');
...
#		}
	...
	public function set_cookie($name = '', $value = '', $expire = '', $domain = '', $path = '/', $prefix = '', $secure = FALSE, $httponly = FALSE)
	{
		...
		if ($prefix === '' && conf('cookie_prefix') !== '')
		{
			$prefix = conf('cookie_prefix');
		}

		if ($domain == '' && conf('cookie_domain') != '')
		{
			$domain = conf('cookie_domain');
		}

		if ($path === '/' && conf('cookie_path') !== '/')
		{
			$path = conf('cookie_path');
		}

		if ($secure === FALSE && conf('cookie_secure') !== FALSE)
		{
			$secure = conf('cookie_secure');
		}

		if ($httponly === FALSE && conf('cookie_httponly') !== FALSE)
		{
