...
require_once dirname(__FILE__)."/profy_aliases.php";

// Translation of the given data
if (!function_exists("t")) {
	function t ($string, $args = 0, $lang = "") {
		if (is_array($string)) {
			foreach ((array)$string as $k => $v) {
				$string[$k] = t($v, $args, $lang);
			}
			return $string;
		}
...
if ((double)phpversion() < 5.0 && FRAMEWORK_IS_COMPILED != 1) {
	require_once (PF_PATH."share/functions/profy_funcs_php4_compat.php");
}
