...
class yf_db_admin {
	...
	*/
	...
	function database_show() {
		...
		$attr_postload_url = 'postload-url';
		$tr_id_key = 'name';
		...
		return _class('html')->tabs(array(
			'tables' => table(
				function() use ($db, $db_name) {
					...
					'table_attr' => 'data-postload-url="'._prepare_html(url_admin('/@object/database_show_ajax/'.$db_name.'/')).'"',
#					'data-postload-url' => 'data-postload-url="'._prepare_html(url_admin('/@object/database_show_ajax/'.$db_name.'/')).'"',
					'tr' => function($row, $_id) {
						return ' id="tr_'.$_id.'"';
					},
#					'th' => function($_id) {
