	...
	function show () {
		...
		while ($A = db()->fetch_assoc($Q)) {
			...
			if ($is_in_framework) {
				$locations[] = array(
					"name"	=> "framework",
					"link"	=> "./?object=file_manager&action=edit_item&f_="."profy_".$A["name"].".class.php"."&dir_name=".urlencode(PF_PATH. "modules"),
				);
			}
			if ($is_in_framework2) {
				$locations[] = array(
					"name"	=> "framework_p2",
					"link"	=> "./?object=file_manager&action=edit_item&f_="."profy_".$A["name"].".class.php"."&dir_name=".urlencode(PF_PATH. "priority2/modules"),
				);
			}
			$replace2 = array(
	...
	*/
	function _recursive_get_methods_from_extends ($file_text = "", $user_module_name = "", $_type = "admin", $ONLY_PRIVATE_METHODS = false) {
		...
		if (preg_match($this->_extends_pattern, $file_text, $matches_extends)) {
			...
			// Special processing of the "profy_module"
