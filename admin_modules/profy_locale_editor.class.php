	...
	function _parse_source_code_for_vars ($params = array()) {
		...
		if (!$params["only_project"]) {
			if (!$params["only_stpls"]) {
				$profy_framework_php_files	= _class("dir")->scan_dir(PF_PATH, true, $this->_include_php_pattern, $this->_exclude_pattern);
			}
			if (!$params["only_php"]) {
				$profy_framework_stpl_files	= _class("dir")->scan_dir(PF_PATH, true, $this->_include_stpl_pattern, $this->_exclude_pattern);
			}
		}
		...
		foreach ((array)$profy_framework_php_files as $file_name) {
		...
		foreach ((array)$profy_framework_stpl_files as $file_name) {
			// Create short file name
			$short_file_name = str_replace(array(REAL_PATH, INCLUDE_PATH, PF_PATH), "", $file_name);
			// Merge vars
