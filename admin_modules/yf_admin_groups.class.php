...
class yf_admin_groups {
	...
	*/
	function delete() {
		...
		if (!empty($_GET['id'])) {
			db()->query('DELETE FROM '.db(self::table).' WHERE id='.intval($_GET['id']).' LIMIT 1');
	...
	*/
	function active() {
		$_GET['id'] = intval($_GET['id']);
		if (!empty($_GET['id'])) {
			$group_info = db()->query_fetch('SELECT * FROM '.db(self::table).' WHERE id='.intval($_GET['id']));
		}
		if ($_GET['id'] == 1) {
			$group_info = [];
		}
		if (!empty($group_info)) {
			db()->update_safe(self::table, ['active' => intval(!$group_info['active'])], 'id='.intval($_GET['id']));
