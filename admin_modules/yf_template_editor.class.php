...
#			"project_p2"	=> P_TPLS_LIB_PATH2,
	...
	*/
	function _show_themes () {
		...
		// Process records
		foreach ((array)$themes as $theme_class => $theme_attr) {
			if (realpath(P_TPLS_LIB_PATH) == realpath($this->_dir_array[$theme_class]) && $theme_class != "project") {
				continue;
			}
			$replace3 = array(
	...
	function _get_themes () {
		...
		foreach ((array)$this->_dir_array as $k => $d){
			...
			while (false !== ($f = readdir($dh))) {
				$dirName = $d.$f;
				if (false !== strpos($dirName, ".svn")) {
					continue;
				}
				if (false !== strpos($dirName, ".git")) {
					continue;
				}
				if (is_dir($dirName) && $f != "." && $f != "..") {
					$themes[$k][$d. $f. "/"] = $f;
				}
			}
		}
