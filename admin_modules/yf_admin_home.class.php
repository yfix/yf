...
class yf_admin_home {
	...
	*/
	function show () {
		...
		return tpl()->parse(__CLASS__.'/new_home', $replace);
	}
	...
	*/
	function _hook_side_column () {
		...
		$replace = array(
			...
			'save_link'		=> './?object='.$_GET['object'].'&action=show',
		);
		return tpl()->parse(__CLASS__.'/new_home_side', $replace);
	}

	/**
	*/
	function _get_dashboard_data () {
		if (isset($this->_dashboard_data)) {
			return $this->_dashboard_data;
		}
		$name = 'admin_home';
		$sql = 'SELECT * FROM '.db('dashboards').' WHERE name="'.db()->es($name).'"';
		$dashboard = db()->get($sql);
		if (!$dashboard) {
		...
		$this->_dashboard_data = $dashboard;
		return $dashboard;
	}
